...
module SystemFOmega.Term2 where

open import SystemFOmega.Type2
  hiding (Con; _∈_; ren-∈; id; Ne; η-Ne; η; Sp; ren;
          renSp; sub; drop;  ∈-eq; subSp; subᶜ; _◇_; drop-sub-⊆; inst)
import SystemFOmega.Type2 as T
...
lower : ∀ {Γ}{Δ : Con Γ}{A}(v : A ∈ Δ) → Nf (dropᶜᵗ v) (dropᵗ v) → Nf (subᶜᵗ v) A
lower v t = subst (Nf _) (ren-drop v) (ren (drop-sub-⊆ v) t)
...
mutual
  ...
  subₖ {Δ = Δ} v t' (ne {v' , sp} (_,_ {A = A} tmv' tmsp)) with T.∈-eq v v'
  ... | inj₁ refl = {!!}
  ... | inj₂ v''  = ne {!!}
...
mutual
  {-# TERMINATING #-} -- and why?? The call graph is the same as in STLC
  subₜ :
